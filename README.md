...
приоритетов и месячных планов. Главная страница — интерфейс месяца в деморежиме.
Можно добавлять приоритеты, менять планы, переключать месяцы и работать с архивом.
Данные деморежима хранятся только в памяти до перезагрузки страницы.
...
Демоэкран работает без облачных настроек и не обращается к базе данных.
Подключение реальных данных запланировано вместе с авторизацией.
Для Neon заполните в `.env.local`:
