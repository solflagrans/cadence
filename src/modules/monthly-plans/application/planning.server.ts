...
import { createPriority, createPriorityInputSchema, priorityNameSchema, prioritySchema } from "@/src/modules/priorities/domain/priority";
import { monthlyPlans, planningOperations } from "../data/schema";
import { createMonthlyPlan, monthlyPlanSchema, monthSchema, targetSchema } from "../domain/monthly-plan";
...
const requestSchema = z.strictObject({ operationId: z.uuid(), command: commandSchema });
const ownerSchema = z.string().trim().min(1);
const storedVersion = z.number().int().positive().max(2_147_483_647);
const priorityViewSchema = prioritySchema.extend({ version: storedVersion });
const planViewSchema = monthlyPlanSchema.extend({ version: storedVersion });
